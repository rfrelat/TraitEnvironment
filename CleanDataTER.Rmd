...
Ideally, the number of variables per dimensions would be somehow balanced. Else, one need to know that some dimensions are given more importance.
...
The table `trait` contain `r ncol(trait)` variables (in column) characterizing `r nrow(trait)` taxa (in rows). Among these variables, there is:  
...
Boxplot represent the key distribution statistics (quartiles) in a graph. The middle bold line represent the median, separating the dataset in two with half the dataset with higher values than the median. The gray box represent the first and third quartiles (i.e. the interquartile range, IQR), i.e. half the dataset are included in this box, 25% have higher value than the third quartile (Q3), and the other fourth have lower values than the first quartile (Q1). The outliers are represented as dots if they are higher than $Q3 +1.5*IQR$ or lower than $Q1-1.5*IQR$.  

In both case, the distribution of values are well spread, and tropic level doesn't require data transformation.
...
```
Most species have offspring size lower than 4 mm (Q3), while few species have very large offspring (max 900 mm). Hence it may be justified to transform this variable.  
...
The same is true for fecundity and we will log-transform it as well.
```{r, message=FALSE}
...
We must first select the continuous variables (can't display categorical variables).
```{r, message=FALSE}
...
Length infinity is strongly positively correlated with maximum length (rho >0.7), and negatively with growth coefficient (rho <-0.75). We will then remove length infinity for the following TER analysis. The rest is *acceptable* (note: there is no rule set in stone for *acceptable* level of correlation).
...
The dataset is largely dominated by non-guarder. The three categories are non equally distributed, but the number of category is reasonably low and each category is represented by at least 62 taxa. However the issue here are the NAs, which in most cases are not tolerated in analysis.
...
Habitat is the typical example of a variable with too many categories. The fact that there is only 9 taxa as non-pelagic, and 22 as reef-associated will give a relatively large weight to these taxa. It might be good to define broader categories (when possible) or simply discard this categorical variable.

#### 2.5 Simplify the trait dataset

For simplicity we decided to keep only seven continuous variables. It is a good idea to keep the name of species as names of the rows to help matching the rows in a later stage.
...
sdmpredictors is a comprehensive list of spatially defined environmental drivers (but considered constant in time, impossible to study temporal dynamics)
Other sources of temporally resolved dataset is Copernicus. Please visit xx for more information on how to extract spatially and temporally resolved data in R.
...
The table `env` contain `r ncol(env)` variables (in column) characterizing `r nrow(env)` sites (in rows). Among these variables, there is: 
...
Same principle applies for the environmental variables as in the trait variables. To have an overview of the variables, we can create a boxplot of the variables scaled (so they have no difference in unit). 
...
The distribution of SBS is not wide, but can't say that it is bad.

### 3.2 Maping the environement variables

Another interesting feature is to look at the spatial distribution of environmental variables (to visually check if there are any errors).
...
Rare species can have a strong role in the TER analysis and keeping them or not is tricky. Additionally, many rare species are also absent from trait database, so it is a fine line.
...
Same pattern here, most of the species have low abundance.

We could cross these information to check the relationship between occurrence and total abundance in a log-scale plot (parameter `log="xy"`). Each dot represent one species.
...
There are no golden rule to decide which species to keep or which one to discard. It depends on the analysis (it it strongly influenced by rare species or not) and on the objectives of the study. Whatever the choice, it must be reported in the methodology. 
...
One last check, we want to see if there are NAs in our tables (which might limit the choice of method). Because our dataset is complete, we did not spend much time about this. But in most cases, it could be one of the most restricting factor when selecting species and sites. Let's see how many NAs there is per variable.
...
It would be wise to recheck the correlation and everything we did before on this reduced set of species and sites. 
...
These are some of the considerations that need to tackle. Now you are ready to investigate your trait environment-relationship in your dataset. There are multiple methods, that differ slightly in their aim and hypothesis. Among them:

1. Computing aggregated indicators of community weighted mean traits [(CWM)](https://rfrelat.github.io/CWM.html) and diversity indicators and link them to environmental variables.
...
3. Hierarchical Modelling of Species Communities [(HMSC)](https://rfrelat.github.io/HMSC.html)
